...
class ViLT_LXMERT_Fusion(nn.Module):
    def __init__(self, num_answers=1000, hidden_dim=768, fusion_dim=1024):
        ...
        # Freeze base encoders if desired for faster convergence
    ...
    def forward(self, vilt_inputs, lxmert_inputs):
        ...
        vilt_emb = vilt_outputs.pooler_output  # [batch, 768]

        # Add dummy visual features for LXMERT
        batch_size = lxmert_inputs["input_ids"].shape[0]
        device = lxmert_inputs["input_ids"].device
        dummy_visual_feats = torch.zeros((batch_size, 36, 2048), device=device)  # [batch, num_objs, feat_dim]
        dummy_visual_pos = torch.zeros((batch_size, 36, 4), device=device)       # [batch, num_objs, bbox coords]
        ...
        lxmert_emb = lxmert_outputs.pooled_output  # [batch, 768]
