...
base_dir = "Dataset"
Val_csv_path = os.path.join(base_dir, "dataset_val2014.csv")
cp_zip_path = os.path.join(base_dir, "v2_Complementary_Pairs_Val_mscoco.zip")
cp_json_path = os.path.join(base_dir, "v2_mscoco_val2014_complementary_pairs.json")

# === Step 1: Load the existing Valing CSV ===
print("Loading existing Valing CSV...")
Val_df = pd.read_csv(Val_csv_path)
print(f"Loaded {len(Val_df)} samples from {Val_csv_path}")
...
questions_path = os.path.join(base_dir, "questions_val/v2_OpenEnded_mscoco_val2014_questions.json")
annotations_path = os.path.join(base_dir, "annotations_val/v2_mscoco_val2014_annotations.json")
...
for pair in cp_pairs:
    ...
    for qid in pair:
        ...
        answer = a_lookup[qid]
        image_path = f"val2014/COCO_val2014_{img_id:012d}.jpg"
        cp_records.append({
...
# === Step 7: Reorder columns to match Valing CSV ===
cp_df = cp_df[["image_path", "question", "answer_idx", "image_id", "question_id", "answer", "is_cp"]]

# Add is_cp=0 to Val_df for clarity
Val_df["is_cp"] = 0

# === Step 8: Merge and save ===
merged_df = pd.concat([Val_df, cp_df], ignore_index=True)
merged_csv_path = os.path.join(base_dir, "dataset_Val2014_with_cp.csv")
merged_df.to_csv(merged_csv_path, index=False)

print(f"✅ Saved merged CSV with complementary pairs to {merged_csv_path}")
print(f"Total rows: {len(merged_df)} (original {len(Val_df)} + {len(cp_df)})")